...
MIN_AREA = 1000 # Minimum area of objects to consider for vision
BALL_PICKUP_SIZE = 3700
...
func2 = detect2Color
args1 = ((np.array([13, 150, 85]), np.array([28, 255, 255])),)
...
while not rospy.is_shutdown():
    ...
    if currState == States.INIT:
        ...
        robot.setJointPos(["head_pan", "head_tilt"], [new_x, -0.8])
        ...
        if tick_count > tickrate:
            ...
            if robot.joint_pos["head_pan"] > -0.2 and robot.joint_pos["head_pan"] < 0.2:
                if area > 4400:
                    robot.onlineWalkSetup(x=0.02, foot_dist=0.08, foot_height=0.03)
        ...
        if pan > 0.05:
            ...
            th = 0.0
        robot.walkVelocities(x=3.0, th=th)

        if area > BALL_PICKUP_SIZE:
            tick_count += 1
